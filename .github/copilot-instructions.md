...
- The runner’s directory scan is **non-recursive** (see `_get_test_files()` in `src/runner.cc`). Point `--test_file_directory` directly at a folder of `.data` files (e.g., `tests` or `tests/rfc9669`).
