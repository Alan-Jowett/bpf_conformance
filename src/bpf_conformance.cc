 ...
 */
std::vector<std::filesystem::path> get_test_files(const std::filesystem::path &test_file_path)
{
 ...
 */
std::string base_16_encode(const std::vector<uint8_t> &data)
{
 ...
 */
std::vector<uint8_t> ebpf_inst_to_byte_vector(const std::vector<ebpf_inst> &instructions)
{
...
int main(int argc, char **argv)
{
    try
    {
        ...
        for (auto &test : tests)
        {
            ...
            boost::process::child c(plugin_path, base_16_encode(input_memory), plugin_options, boost::process::std_out > output, boost::process::std_in < input);
